                ...
                this.leaveMngmtService.getMyLeaveStatus().subscribe((data: any) => {
                    ...
                    if (this.leaveInfo.rollover != undefined && this.company.rollover_max_day != undefined) {
                        ...
                        this.dataService.userProfile.pipe(takeUntil(this.unsubscribe$)).subscribe((data: any) => {
                            ...
                            this.rolloverMinDate = moment(data.emp_start_date)
                                .add(careerYear, 'y')
                                .format('YYYY-MM-DD');
                            this.rolloverMaxDate = moment(this.rolloverMinDate)
    ...
    errorAlert(err) {
        switch (err) {
            ...
            case 'DB Error':
                this.dialogService.openDialogNegative('An error has occurred while requesting');
                break;
            case 'no next annual leave':
                this.dialogService.openDialogNegative('No next annual leave');
                break;
        }
    }
