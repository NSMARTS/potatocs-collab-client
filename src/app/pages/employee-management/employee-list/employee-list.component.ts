...
export class EmployeeListComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.dataService.userCompanyProfile.pipe(takeUntil(this.unsubscribe$)).subscribe((data: any) => {
                ...
                if (data.message == 'found') {
                    ...
                    if (this.isRollover) {
                        for (let index = 0; index < data.myEmployeeList.length; index++) {
                            data.myEmployeeList[index].totalLeave.rollover = Math.min(
                                data.myEmployeeList[index].totalLeave.rollover,
                                this.company_max_day,
                            );
                        }
                    }
                    ...
                    console.log('휴가봐야함', this.getMyEmployeeList.data);
                }
            },
            err => {
                console.log(err);
